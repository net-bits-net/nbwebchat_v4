...
function ToggleAwayButton(sReason, src) {
	...
	if (ouserMe.away == false) {
		ouserMe.away = true;
		if (sReason.length > 0) NBChatController.sendToServer("AWAY " + m_sChan + " " + sReason);
		else NBChatController.sendToServer("AWAY " + m_sChan + " ");
...
function ListView(pListViewParent) {
	...
	function selectItem(lvi) {
		...
		console.log("selectItem#2", selectedItem);
